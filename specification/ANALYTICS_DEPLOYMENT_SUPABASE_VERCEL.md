...
- `SUPABASE_URL`
  - **Where to find it (Supabase UI):** Project → **Project Settings** (gear) → **API** → **Project URL**
  - Looks like: `https://<project-ref>.supabase.co`

- `SUPABASE_SERVICE_ROLE_KEY`
  - **Where to find it (Supabase UI):** Project → **Project Settings** → **API** → **Project API keys** → `service_role`
  - This key is generated by Supabase (you don’t define it yourself).
  - **Security:** this key has elevated privileges and bypasses RLS. Store it only in **Vercel Environment Variables** (server-side) and never expose it in the frontend.
